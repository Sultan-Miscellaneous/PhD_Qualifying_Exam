...
@block
def datamover(identifier, clk, enable, data, port, program, done, cycle_program = False, repeat = 1, mode = 'read'):
...
def datamover_tb():
    ...
    memory = Memory(1, 1, 200, initialization_vals=initialization_list, size=memory_size)
    ...
    def stimulus():
        ...
        for idx in range(100):
            if memory.storage[idx] != memory.storage[idx+100]:
                raise Exception("Datamover testbench failed, mismatch in memory... aborting...")
